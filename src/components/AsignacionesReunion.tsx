...
// Main Component
export const AsignacionesReunion: React.FC<AsignacionesReunionProps> = ({
    publishers,
    schedules,
    onSaveSchedule,
    onShowModal,
        ...
        try {
            ...
            for (const meetingDate of allMeetingDates) {
                ...
                if (isMidweek) {
                    ...
                    const conductorAtalayaArr = getNextAvailable('Conductor de la Atalaya', 1);
                    assignmentsForDay.conductorAtalaya = conductorAtalayaArr[0] ?? undefined;
                    const presidenteArr = getNextAvailable('Presidente', 1);
                    assignmentsForDay.presidente = presidenteArr[0] ?? undefined;
                    const lectorAtalayaArr = getNextAvailable('Lector de la Atalaya', 1);
                    assignmentsForDay.lectorAtalaya = lectorAtalayaArr[0] ?? undefined;
                    assignmentsForDay.vigilantes = getNextAvailable('Vigilante', 3);
                    assignmentsForDay.acomodadoresSala = getNextAvailable('Acomodador de los Asistentes', 2);
                    assignmentsForDay.microfonos = getNextAvailable('Micrófono', 2);
                    assignmentsForDay.acomodadoresPrincipal = getNextAvailable('Acomodador en la puerta Principal', 1);
    ...
    const ScheduleView = ({ schedule, isEditing }: { schedule: MeetingAssignmentSchedule | null, isEditing: boolean }) => {
        ...
        const CardAssignmentRow: React.FC<{ label: string; children: React.ReactNode }> = ({ label, children }) => (
            <div className="flex justify-between items-center py-1.5 border-b border-gray-100 last:border-b-0">
                <span className="font-medium text-gray-600">{label}:</span>
                <div className="text-right w-1/2">{children}</div>
            </div>
        );

        const ABBR_TO_ROLE_NAME: Record<string, string> = {
            'AP': 'P. Principal',
            'APA': 'P. Auditorio',
            'AA': 'Asistentes',
            'AM': 'Micrófonos',
                        ...
                        <div key={week.weekDateRange} className="bg-white p-4 rounded-lg shadow-md border">
                            ...
                            <div className="mb-4">
                                ...
                                <div className="space-y-1 text-sm">
                                    <CardAssignmentRow label="Presidente">{isEditing ? <select value={week.presidente.id || ''} onChange={e => handleEditChange(week.weekendDateKey, 'presidente', e.target.value)} className="w-full p-1 border rounded text-xs bg-yellow-50"><option value="">--</option>{getEligiblePublishers('Presidente').map(p => <option key={p.id} value={p.id}>{getPublisherName(p.id)}</option>)}</select> : week.presidente.name}</CardAssignmentRow>
                                    <CardAssignmentRow label="Lector Atalaya">{isEditing ? <select value={week.lectorAtalaya.id || ''} onChange={e => handleEditChange(week.weekendDateKey, 'lectorAtalaya', e.target.value)} className="w-full p-1 border rounded text-xs bg-yellow-50"><option value="">--</option>{getEligiblePublishers('Lector de la Atalaya').map(p => <option key={p.id} value={p.id}>{getPublisherName(p.id)}</option>)}</select> : week.lectorAtalaya.name}</CardAssignmentRow>
                                    {week.weekendAssignments.map(item => (
                                    ...
                                    <CardAssignmentRow label="Aseo">{isEditing ? <select value={week.aseo.id || ''} onChange={e => handleEditChange(week.weekendDateKey, 'aseo', e.target.value)} className="w-full p-1 border rounded text-xs bg-yellow-50"><option value="">--</option>{groups.map(g => <option key={g} value={g}>Grupo {g}</option>)}</select> : week.aseo.name}</CardAssignmentRow>
