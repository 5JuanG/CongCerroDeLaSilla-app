...
interface GestionContenidoProps {
    ...
    onAddInvitation: (imageFile: Blob, phrase: string) => Promise<void>;
    onDeleteInvitation: (contentId: string) => Promise<void>;

    homepageContent: HomepageContent[];
    onAddHomepageContent: (imageFile: Blob, title: string, phrase: string) => Promise<void>;
...
const GestionContenidoInvitacion: React.FC<GestionContenidoProps> = ({
    ...
    const createSubmitHandler = (type: 'invitation' | 'homepage') => async (e: React.FormEvent) => {
        ...
        try {
            const compressedBlobs: Blob[] = await Promise.all(files.map(file => compressImage(file)));
            ...
            for (let i = 0; i < compressedBlobs.length; i++) {
                ...
                try {
                    if (type === 'invitation') {
                        await onAddInvitation(blob, phrase.trim());
                    } else {
                        await onAddHomepageContent(blob, title.trim(), phrase.trim());
                    }
                    successCount++;
                } catch (uploadError) {
            ...
            if (failedFiles.length > 0) {
                onShowModal({
                    ...
                    message: `Se subieron ${successCount} de ${compressedBlobs.length} imágenes.\n\nFallaron: ${failedFiles.join(', ')}`
                });
            } else {
                onShowModal({
                    type: 'success',
    ...
    const TabButton: React.FC<{ tabName: typeof activeTab; label: string }> = ({ tabName, label }) => (
        <button
            ...
            className={`whitespace-nowrap py-4 px-1 border-b-2 font-medium text-sm ${activeTab === tabName
